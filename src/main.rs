/// farsi-shell: A PTY wrapper for Termux that displays Persian/Arabic text correctly
///
/// Usage: farsi-shell [shell]
///   shell: Path to shell to use (default: $SHELL or /data/data/com.termux/files/usr/bin/bash)
...
fn main() {
    ...
    eprintln!("╔══════════════════════════════════════════════════════════╗");
    eprintln!("║  farsi-shell v{:<41}║", VERSION);
    eprintln!("║  Persian/Arabic text display for Termux                 ║");
    eprintln!("╠══════════════════════════════════════════════════════════╣");
    eprintln!("║  ✓ Character reshaping (connecting letters)             ║");
    eprintln!("║  ✓ BiDi text reordering (RTL support)                   ║");
    eprintln!("║  ✓ ANSI escape sequence preservation                    ║");
    eprintln!("╠══════════════════════════════════════════════════════════╣");
    ...
    let session = match PtySession::new(shell, win_size) {
        Ok(session) => session,
        Err(e) => {
            eprintln!("farsi-shell: failed to create PTY: {}", e);
            process::exit(1);
        }
    };
...
fn print_help() {
    ...
    println!();
    println!("USAGE:");
    println!("    farsi-shell [OPTIONS] [SHELL]");
    println!();
    println!("OPTIONS:");
    ...
    println!();
    println!("ARGUMENTS:");
    println!("    SHELL            Path to shell (default: $SHELL)");
    println!();
    println!("EXAMPLES:");
    println!("    farsi-shell");
