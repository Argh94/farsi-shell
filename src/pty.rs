...
impl PtySession {
    pub fn new(shell: Option<&str>, win_size: WinSize) -> io::Result<Self> {
        ...
        let original_termios = termios::tcgetattr(stdin_borrowed).ok();

        unsafe {
            match fork().map_err(|e| io::Error::new(io::ErrorKind::Other, format!("fork failed: {}", e)))? {
                ForkResult::Parent { child } => {
                    close(slave_fd).ok();
                    if let Ok(mut t) = termios::tcgetattr(stdin_borrowed) {
                        set_raw_mode(&mut t);
                        let _ = termios::tcsetattr(stdin_borrowed, SetArg::TCSANOW, &t);
                    }
