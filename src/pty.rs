use std::io;
use std::os::unix::io::{AsRawFd, RawFd, BorrowedFd, FromRawFd, OwnedFd};
use nix::pty::openpty;
use nix::unistd::{fork, ForkResult, setsid, dup2, close, Pid, execvp};
    ...
    pub fn new(shell: Option<&str>, win_size: WinSize) -> io::Result<Self> {
        ...
        match unsafe { fork() } {
            Ok(ForkResult::Parent { child }) => {
                ...
                // Put parent terminal in raw mode
            ...
            Ok(ForkResult::Child) => {
                ...
                #[cfg(target_os = "linux")]
                unsafe { libc::ioctl(slave_fd, 0x540E, 0); }
                ...
                // Execute shell
                let shell_cstr = CString::new(shell_path).expect("null byte in shell path");
                let _ = execvp(shell_cstr.as_c_str(), &[shell_cstr.as_c_str()]);
