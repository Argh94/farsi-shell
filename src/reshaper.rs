...
pub enum JoiningType { Dual, Right, None, Transparent }
...
pub fn is_arabic_letter(ch: char) -> bool {
    ...
    matches!(cp,
        ...
        0x0671 | 0x0679..=0x067D | 0x0680..=0x069C |
        0x06A0..=0x06B9 | 0x06BB..=0x06D3 | 0x06D5 |
        0x06EE..=0x06EF | 0x06F0..=0x06F9 |
        0x0750..=0x077F | 0x08A0..=0x08FF
    )
}
...
pub fn is_diacritic(ch: char) -> bool {
    ...
    matches!(cp,
        ...
        0x06EA..=0x06ED | 0x08D4..=0x08E1 | 0x08F0..=0x08FF
    )
}

pub fn joining_type(ch: char) -> JoiningType {
    if is_diacritic(ch) { return JoiningType::Transparent; }
    match ch {
        '\u{0621}' => JoiningType::None,
        '\u{0622}' | '\u{0623}' | '\u{0624}' | '\u{0625}' |
        '\u{0627}' | '\u{0629}' | '\u{062F}' | '\u{0630}' |
        '\u{0631}' | '\u{0632}' | '\u{0648}' | '\u{0671}' |
        '\u{067E}' | '\u{0698}' | '\u{06CC}' => JoiningType::Right,
...
fn reshape_word(chars: &[char], joining: &[JoiningType]) -> String {
    ...
    for (idx, (&ch, &jt)) in chars.iter().zip(joining.iter()).enumerate() {
        match jt {
            JoiningType::Transparent => { result.push(ch); }
            JoiningType::None => { result.push(get_shaped_char(ch, true, false, false, false)); }
            JoiningType::Right => {
                ...
                if connects_prev { result.push(get_shaped_char(ch, false, false, false, true)); }
                else { result.push(get_shaped_char(ch, true, false, false, false)); }
            }
            JoiningType::Dual => {
                let connects_prev = if idx > 0 {
                    let mut j = idx - 1;
                ...
                let connects_next = if idx < len - 1 {
                    ...
                    loop {
                        ...
                        if joining[j] == JoiningType::Transparent { j += 1; }
                        else { break matches!(joining[j], JoiningType::Dual | JoiningType::Right); }
                    }
                } else { false };
